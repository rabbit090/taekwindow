...
BOOL ConfigSheet::buttonsPageDialogProc(HWND pageHandle, UINT message, WPARAM wParam, LPARAM lParam) {
	switch (message) {
		case WM_INITDIALOG:
			CheckRadioButton(pageHandle, IDC_LEFTALT, IDC_SHIFT_CTRL_ALT,
				...
				d_newConfig.modifier == VK_MY_CTRL_SHIFT ? IDC_SHIFT_CTRL :
				d_newConfig.modifier == VK_MY_CTRL_ALT ? IDC_CTRL_ALT :
				IDC_SHIFT_CTRL_ALT
				);
			CheckRadioButton(pageHandle, IDC_MOVELEFT, IDC_MOVERIGHT,
			...
			switch (((NMHDR*)lParam)->code) {
				case PSN_APPLY:
					d_newConfig.modifier =
						...
						IsDlgButtonChecked(pageHandle, IDC_SHIFT_CTRL) ? VK_MY_CTRL_SHIFT :
						IsDlgButtonChecked(pageHandle, IDC_CTRL_ALT) ? VK_MY_CTRL_ALT :
